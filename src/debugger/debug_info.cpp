...
bool DebugInfo::load(const char *path) {
	...
	for (uint32_t i = 0; i < nr_sections; i++) {
		...
		if (tag == "SRCS") {
			if (!load_srcs(srcs, section_content)) return false;
		} else if (tag == "SCNT") {
			if (!load_scnt(srcs, section_content)) return false;
		} else if (tag == "LINE") {
			if (!load_line(srcs, section_content)) return false;
		} else if (tag == "VARI") {
			if (!load_vari(variables, section_content)) return false;
		} else {
